...
var Active_elevators = make(map[string]ElevInfo)
...
func InitQueuemanager(ipaddr string, c_router_info chan []byte, c_to_statemachine chan int, c_peerListUpdate chan string, c_queMan_output chan []byte, c_queMan_ack_order chan []byte) {
	...
	new_elevator := ElevInfo{my_ipaddr, false, false, false, false, 0, 0, 0,false, -1, -1, my_ordermatrix}
...
func CostFunction(elevator_ip string, order_floor int, button_dir string) int {
	...
	*/	
	...
	if current_elevator.MOVING {
		current_elevator.POSITION = current_elevator.POSITION*2 + current_elevator.DIRECTION
		if current_elevator.POSITION == -1 {
			current_elevator.POSITION = 0
		}else if current_elevator.POSITION == N_FLOORS*2-1{
			current_elevator.POSITION = current_elevator.POSITION*2
		}
	}else{
		current_elevator.POSITION = current_elevator.POSITION*2
	}
...
func processNewInfo(c_router_info chan []byte, c_peerListUpdate chan string, c_queMan_output chan []byte, c_queMan_ack_order chan []byte) { //, c_pos_from_statemachine chan int, c_dir_from_statemachine chan int){
	...
	for {
		...
		case encoded_elev_info := <-c_router_info:
			...
			fmt.Println("queue", elev_info)
			if _, in_list := Active_elevators[elev_info.IPADDR]; !in_list {
<<<<<<< HEAD
				AppendElevator(elev_info)
=======
				Active_elevators[elev_info.IPADDR] = elev_info
				fmt.Println("Elevator", elev_info.IPADDR, "online\n")			
>>>>>>> cab4c30ea1d6b11aaa3f6c2e419fb92f26703a87
			}

			if elev_info.F_NEW_INFO && !compare_ElevInfo(elev_info, last_info) {
...
>>>>>>> cab4c30ea1d6b11aaa3f6c2e419fb92f26703a87
				...
				} else if pos == Active_elevators[my_ipaddr].DESTINATION && Active_elevators[my_ipaddr].ORDER_MATRIX[i][0] == 1 && i > pos {
					dest = i
					fmt.Println("queue: New destination floor: ", dest)
					temp_elev := Active_elevators[my_ipaddr]
					temp_elev.DESTINATION = dest
...
>>>>>>> cab4c30ea1d6b11aaa3f6c2e419fb92f26703a87
				...
				} else if pos == Active_elevators[my_ipaddr].DESTINATION && Active_elevators[my_ipaddr].ORDER_MATRIX[i][1] == 1 && i < pos {
					dest = i
					fmt.Println("queue: New destination floor: ", dest)
					temp_elev := Active_elevators[my_ipaddr]
					temp_elev.DESTINATION = dest
...
func compare_ElevInfo(new_info ElevInfo, old_info ElevInfo) bool{
	if new_info.IPADDR 		== old_info.IPADDR &&
	new_info.F_NEW_INFO 	== old_info.F_NEW_INFO &&
	new_info.IPADDR 		== old_info.IPADDR &&
	new_info.F_BUTTONPRESS 	== old_info.F_BUTTONPRESS &&
	new_info.F_ACK_ORDER 	== old_info.F_ACK_ORDER &&
	new_info.POSITION 		== old_info.POSITION &&
	new_info.DIRECTION 		== old_info.DIRECTION &&
	new_info.DESTINATION 	== old_info.DESTINATION &&
	new_info.MOVING 		== old_info.MOVING &&
	new_info.BUTTON_TYPE 	== old_info.BUTTON_TYPE &&
	new_info.BUTTONFLOOR 	== old_info.BUTTONFLOOR {
		return true
	}else{
		return false
	}
